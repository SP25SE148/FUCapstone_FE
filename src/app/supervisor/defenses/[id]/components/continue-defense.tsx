...
interface ContinueDefenseProps {
  ...
  defendCapstoneAttempt: number;
}

const ContinueDefense: React.FC<ContinueDefenseProps> = ({
  defendCapstoneCalendarId,
  defendCapstoneAttempt,
}) => {
  const redefense = defendCapstoneAttempt === 2;
  const { updatePresidentDecisionForGroupStatus, getThesisPresignedUrl } =
    useSupervisorDefense();
  const router = useRouter();
  const [open, setOpen] = useState<boolean>(false);
  const [selectedOption, setSelectedOption] = useState(redefense ? "Finish" : "2nd Defense");
